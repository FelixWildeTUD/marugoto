...
def trainWithAugmentation(
    *,
    bags: Sequence[Iterable[Path]],
    augmented_bags: Sequence[Iterable[Path]],
    targets: Tuple[SKLearnEncoder, npt.NDArray],
    add_features: Iterable[Tuple[SKLearnEncoder, npt.NDArray]] = [],
    valid_idxs: npt.NDArray[np.int_],
    n_epoch: int = 32,
    ...
    """Train a MLP on image features with Augmentation.
    ...
    augmented_bags = augmented_bags[valid_idxs.index]

    train_ds = make_dataset(
        # type: ignore  # arrays cannot be used a slices yet
        bags=np.concatenate((bags[~valid_idxs], augmented_bags[~valid_idxs])),
        targets=(target_enc, np.concatenate(
            (targs[~valid_idxs], targs[~valid_idxs]))),
        add_features=[(enc, vals[~valid_idxs]) for enc, vals in add_features],
        bag_size=None,
    )
